...
import * as lacchain from "@lacchain/gas-model-provider";
...
export default class DIDRegistry {
  constructor(conf = {}) {
    ...
    this.registry = new ethers.Contract(
      conf.registry,
      conf.nodeAddress ? DIDRegistryContract.abi : DIDRegistryContractOld.abi,
      provider
    );
  }
  ...
  configureProvider(conf = {}) {
    if (conf.web3) {
      ...
      if (conf.nodeAddress) {
        ...
        return new lacchain.GasModelSigner(
          conf.controllerPrivateKey,
          provider,
          conf.nodeAddress,
          conf.expiration
        );
  ...
  async getControllers(address) {
    if (this.conf.nodeAddress) return this.registry.getControllers(address);
  ...
  async changeControllerSigned(address, controllerPrivateKey, newController) {
    ...
    const sig = await signData(
      ...
      Buffer.from("changeController").toString("hex") + stripHexPrefix(newController),
      nonce.toNumber(),
      this.conf.registry
    );
    return await this.registry.changeControllerSigned(address, sig.v, sig.r, sig.s, newController, {
      gasLimit: 1000000,
      gasPrice: 0,
    });
  }
  ...
  async revokeAttribute(address, key, value) {
    return this.registry.revokeAttribute(address, stringToBytes(key), attributeToHex(key, value), {
      gasLimit: 1000000,
      gasPrice: 0,
    });
  }
